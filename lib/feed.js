...
// "Indisponivel" no Full nao e tudo igual: internalProcess e o galpao conferindo
// e guardando o que acabou de chegar, e libera sozinho. So o resto pede chamado.
const NOME_STATUS = {
    internalProcess: 'em processamento no galpão',
    notSupported: 'sem suporte para venda',
    damaged: 'avariado',
    lost: 'extraviado',
    withdrawal: 'em retirada',
    noFiscalCoverage: 'sem cobertura fiscal',
};
const nomeStatus = (st) => NOME_STATUS[st] || st;
const emProcesso = (p) => (p.bloqueado_detalhe || [])
    .filter(d => d.status === 'internalProcess').reduce((a, d) => a + Number(d.quantidade || 0), 0);
const retido = (p) => Math.max(0, Number(p.bloqueado || 0) - emProcesso(p));
const statusRetidos = (p) => (p.bloqueado_detalhe || [])
    .filter(d => d.status !== 'internalProcess').map(d => nomeStatus(d.status)).join(', ') || 'retido pelo ML';
...
function metricas(p) {
    ...
    if ((p.bloqueado || 0) > 0) {
        m.push({ rotulo: 'Travado', valor: String(p.bloqueado), nota: (p.bloqueado_detalhe || []).map(d => nomeStatus(d.status)).join(', ') || 'retido pelo ML' });
    }
...
const FILAS = [
    {
        ...
        acao: (p) => retido(p) > 0
            ? {
                ...
                detalhe: `${un(retido(p))} ${retido(p) === 1 ? 'retida' : 'retidas'}: ${statusRetidos(p)}`,
                motivo: `O galpão tem a mercadoria, mas marcou como indisponível. Não é falta de estoque: é liberação.`,
            }
            : {
                verbo: 'Aguardar o ML',
                detalhe: `${un(emProcesso(p))} em processamento no galpão`,
                motivo: `Chegaram ao Full e o ML ainda está conferindo e guardando. Costuma liberar sozinho em poucos dias — só abra chamado se continuar assim por mais de uma semana.`,
            },
